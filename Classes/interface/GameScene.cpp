...
void GameScene::showGameBg()
{
    ...
    m_pshotlightaction = (ActionInterval*)GameInfo::getInstance()->createAnitmation("shoting", 27, 1, 0.05);
    m_pshotlightaction->retain();
    
    m_prunlightaction = GameInfo::getInstance()->createAnitmation("light", 10, -1, 0.1);
...
void GameScene::shotlight()
{
    ...
    m_pshotlight->runAction(Sequence::create(m_pshotlightaction,callback, NULL));
...
void GameScene::runupdate(float dt)
{
    ...
    if(m_pruncount >= maxcount && RoleLayer::getInstance()->getrunroleID() == GameData::getInstance()->getprizeid())
    {
